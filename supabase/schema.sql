...
-- One row per team per SDG it addresses (primary and/or secondary).
create view public.v_team_sdg_goal with (security_invoker = true) as
select t.id as team_id, t.event_id, g.sdg_id, g.is_primary,
       o.attainment as sdg_score, r.pct as total_pct
from public.teams t
cross join lateral (values (t.primary_sdg, true), (t.secondary_sdg, false)) as g(sdg_id, is_primary)
left join public.v_team_outcome o on o.team_id = t.id and o.outcome_code = 'SDG'
...
-- Goal-wise SDG attainment per event (all 17 goals listed).
create view public.v_sdg_goal_event with (security_invoker = true) as
select x.*, public.attainment_level(x.pct_at_target) as level
from (
  select e.id as event_id, sd.id as sdg_id, sd.name, sd.color,
         count(g.team_id)  as teams,
         avg(g.sdg_score)  as avg_sdg_score,
         avg(g.total_pct)  as avg_total_pct,
         (count(*) filter (where g.sdg_score >= s.target))::numeric
           / nullif(count(g.sdg_score), 0) as pct_at_target
  from public.events e
  cross join public.sdgs sd
  cross join public.settings s
  left join public.v_team_sdg_goal g on g.event_id = e.id and g.sdg_id = sd.id
  group by e.id, sd.id, sd.name, sd.color, s.target
) x;

-- Goal-wise SDG attainment, all events pooled.
create view public.v_sdg_goal_overall with (security_invoker = true) as
select x.*, public.attainment_level(x.pct_at_target) as level
from (
  ...
  from public.sdgs sd
  cross join public.settings s
  left join public.v_team_sdg_goal g on g.sdg_id = sd.id
  group by sd.id, sd.name, sd.color, s.target
) x;
...
grant select on public.v_team_criterion, public.v_team_result, public.v_team_outcome,
                public.v_event_outcome, public.v_overall_outcome, public.v_team_sdg_goal,
                public.v_sdg_goal_event, public.v_sdg_goal_overall to authenticated;
...
insert into public.outcomes (code, name, kind, sort) values
 ...
 ('SDG',  'SDG Integration: how well the solution is aligned to, and advances, the chosen Sustainable Development Goal(s)', 'SDG', 14);
...
-- Correlation matrix (3 = high, 2 = medium, 1 = low). Same as the Excel workbook.
insert into public.mapping (criterion_id, outcome_code, weight)
select c, o, w from (values
 (1,'PO1',1),(1,'PO2',3),(1,'PO4',1),(1,'PO6',3),(1,'PO11',1),(1,'PSO2',1),(1,'SDG',3),
 (2,'PO1',2),(2,'PO2',3),(2,'PO4',3),(2,'PO5',1),(2,'PO6',1),(2,'PO11',2),(2,'SDG',1),
 (3,'PO1',2),(3,'PO2',2),(3,'PO3',3),(3,'PO4',1),(3,'PO6',1),(3,'PSO1',2),(3,'PSO2',3),(3,'SDG',1),
 ...
 (6,'PO3',1),(6,'PO6',3),(6,'PO7',3),(6,'PSO2',1),(6,'SDG',3),
 (7,'PO3',1),(7,'PO6',2),(7,'PO10',3),(7,'PSO2',2),(7,'SDG',2),
